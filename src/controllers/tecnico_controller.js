...
const login = async(req,res)=>{
    ...
    if(!verificarPassword) return res.status(404).json({msg:"Lo sentimos, correo o contraseña incorrectos"})
...
const registro = async (req, res) => {
  ...
  try {
    ...
    console.error("Error al guardar el nuevo técnico en la base de datos:", error);
    return res.status(500).json({ msg: "Error al registrar el técnico. Por favor, intente nuevamente más tarde." });
  }
};
...
const verificarAdmin = async (req, res) => {
  ...
  try {
    ...
    if (tecnico.rol === "admin") {
      ...
      return res
        .status(403)
        .json({ msg: "El técnico no tiene el rol de administrador" });
    }
  } catch (error) {
